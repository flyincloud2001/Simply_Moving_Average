...
print(f'{SYMBOL} Performance is: {round(price, 4)}')
...
plt.tight_layout()
plt.savefig(f'{SYMBOL}_with_sma_strategy.png', dpi=150)
plt.show()

fig, ax = plt.subplots(figsize=(12, 5))
ax.plot(tested_data['sma1'], label='sma1')
ax.plot(tested_data['sma2'], label='sma2')
ax.plot(tested_data[SYMBOL], label=f'{SYMBOL}')
ax.legend()
ax.set_title(f'{SYMBOL} time series')
plt.tight_layout()
plt.savefig(f'{SYMBOL}_time_series.png', dpi=150)
plt.show()
